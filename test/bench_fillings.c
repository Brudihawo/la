...
void gen_randoms(long *rows, long *cols, float *vals, long n_vals, long n_rows,
  ...
  for (long i = n_vals - invalid_indices; i < n_vals; ++i) {
    ...
    while (repeat) {
      ...
      long pos = binary_search(&idcs[n_vals - invalid_indices - 1],
                               cur_corr_idx, idcs[i]);
      if (pos > 0) {
        repeat = true;
      }
...
void matrix_product_time(long size, long nvals, long n_runs) {
  ...
  mean /= n_runs;
  fprintf(stderr, "%6ld %6.2f" FPRC " " FPRC " " FPRC "\n", size,
          (float)s_A.nvals * 100.0 / (size * size), min, max, mean);
...
int main(void) {
  srand(69);
  const long n_runs = 10;
  fprintf(stderr, "# SIZE fill%% %11s %11s %11s [%ld iterations per size]\n",
          "Min", "Max", "Mean", n_runs);
  const long size = (long)pow(2, 11);

  for (long i = 1; i < 10; ++i) {
    const long nvals = size * size / 100 * i;
